...
- ヘッダーのボタン（☰）でサイドバーを開閉できるようにした。閉じると本文が左いっぱいまで寄る（本文の最大幅は従来どおり `max-width: 900px` のままで、行長は変わらない）
- 閉じている間はサイドバーの中身をキーボードのタブ順と支援技術のツリーからも外す（列幅0だけでは見た目が隠れるのみでリンクがフォーカス可能なまま残るため）
...
- サイドバーツリーの子検索を索引化し、ページ数の多いスペースでのビルド時間を短縮（400ページの合成コンテンツで 19.9秒 → 0.87秒。生成されるHTMLは変更なし）

### Fixed（サイドバー階層表示）
- 左サイドバーのツリーで、開閉マーカー（▶）が項目名と別の行に表示され1行を無駄にしていた不具合を修正。あわせて、開いた項目のマーカーが▼にならず開閉状態が分からなかった不具合も修正（原因は同じで、`.tree-item` の `display: block` が `summary` の `display: flex` より詳細度で勝っていたため。flex が効かないと ▶ が inline 要素になり、項目名が次の行に落ち、回転用の transform も無視される）
